...
class App extends Component {
  ...
  handleChange(event) {
    ...
    const wordCompleted = checkInputComplete(event.target.value, this.state.passage, this.state.index);
    const wordCorrect = checkWordCorrect(event.target.value, this.state.passage, this.state.index);
    const input = wordCompleted? '' : event.target.value;
    const index =  wordCompleted? this.state.index +1 : this.state.index;
    const numWordsTyped = index;
    const lastWord = LastWordCompleted(event.target.value, this.state.passage, this.state.index);
